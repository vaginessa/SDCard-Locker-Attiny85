...
#define ICRSET 0x11
  USICR = ICRSET;
  USICR = ICRSET; // set with clock low
}
...
static uint8_t ButtonIs (uint8_t state)
{
  ...
  uint8_t i;
  if (ReadSwitchOnce () == state)
    {				// if switch state seems to match
      match = 1;
      for (i = 0; i < 5; i++)
	{			// debounce check every 100ms, 5 times
	  _delay_ms (100);
	  if (ReadSwitchOnce () != state)
	    {			// if state doesn't match now
...
Xchg (uint8_t c)
{
  ...
  for (bit = 0; bit < 8; bit++)
    {				// Loop through 8 bits
      if (c & 0x80)
	SPI_PORT |= (1 << MOSI_BIT);	// If bit(7) of "c" is high
      else
	SPI_PORT &= ~(1 << MOSI_BIT);	// if bit(7) of "c" is low
      SPI_PORT |= (1 << SCK_BIT);	// Serial Clock Rising Edge
      c <<= 1;			// Shift "c" to the left by one bit
      if (SPI_PIN & (1 << MISO_BIT))
	c |= 0x01;		// If bit of slave c is high
      else
	c &= ~0x01;		// if bit of slave c is low
...
static uint8_t SD_send_command (uint8_t command, uint32_t arg)
{
  ...
  uint8_t crc;
  if (command & 0x80)
    {				// special case, ACMD(n) is sent as CMD55 and CMDn
  ...
  for (i = 0; i < 10; i++)
    {				// loop until timeout or response
      response = Xchg (0xff);
      if ((response & 0x80) == 0)
  ...
  if ((command != SD_READ_BLK) &&
      ...
      (command != SD_SEND_CID) && (command != SD_SEND_IF_COND)
...
int main (void)
{
  ...
  while (1)
	...
	{			// If the user presses the button...
	  ...
	  if (CardIsLocked () == prevState)
	    {			// if state did not change as expected
